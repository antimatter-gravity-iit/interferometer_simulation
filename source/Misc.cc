...
double maximumvalue(double arr[], int rows){    // finds the max value of the array
...
double sinc(double x)// this function avoids a division by 0; sinc is just defined to be sin(x)/x
{
...
double ( * ixgenerator(double a[], double zloc, int logchoice, int rows)) // accounts for different 
{
  double cutoff = 0.000001; // at what point does the intensity cut off and be treated as 0. Can also be 5e-5 like in McMorran thesis. Or 0.001.cutoff = 0.000001;
  ...
  int rowsT =41;// rows of ReT and ImT arrays
    ...
    double max = maximumvalue(a, rows); // maximum intensity value for a slice in z
    double min = cutoff; // the minimum value the intensity can be at before being set to 0; cutoff is a global variable
    ...
    double realzloc = zloc / 36.075; // change
...
// Modeling electrons:
      // a[i][1] = a[i][1] * (1 - pow(const_e, (-1 * realzloc / 6300)/mu_lifetime));
       ...
       if (logchoice == 1)
         {
           if (a[i] > 0) {
             ...
             a[i] = log(a[i]/min); // tries to make sure all elements are above 1, then takes the logarithm to scale the stuff better
           }
