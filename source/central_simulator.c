...
/*
 * TODO LAcomment: Clarify origin of variables; ask M about need to keep them.
 * None of these variables are currently used. They can be used for future simulation parameters.
 * 
 * The result of the above equation. Not used in main program.
 *     double mu_debroglie = 0.0000000005584;      
 * Muonium frequency.
 *     double muonium_freq = 536890000000000000;   
 * Half life average decay time of a muon, in seconds.
 *     double mu_lifetime = 0.0000022;             
 * The VdW coefficient for hydrogen (assumed to be the same for muonium).
 *     float C3 = 0.020453;                        
 * Force: m^2/(Coulomb^-2).
 *     double Coulomb = 0.00000000898755179;       
 * The constant irrational number pi.
 *     double pi = M_PI;       
...
/* 
 * TODO LAcomment: Clarify origin and ask M whether we need to keep this comment. 
 * From McMorran's thesis, a value epsilon = ratio of permittivity of grating material to permittivity of free space,
 * and the image charge is  + e  *  (epsilon - 1) / (epsilon  +  1). For an ideal conductor:
 * image charge = + e (electric charge), the energy due to this strength of image charge 1 nm from surface is U = -0.75eV.
 */
 ...
 * The main function takes an integer and an array as arguments.
 * List of elements of argv[#], in the format "<position>. <element>": 
...
int main(int argc, char *argv[]){ 
	...
	/*
	 * TODO LAcomment: make sense and rename everything. Functions in library: BeamParams.h.
	 * Original description: "w1 = coherence width of beam. Using a function w to initialize this variable. 
         * Following three are used to calculate Gaussian Schell-model (GSM) values at the first grating." 
	 */
		
	// TODO: LAcomment: Original desciption: "Calculates first grating coherence width of beam." What does it mean?
