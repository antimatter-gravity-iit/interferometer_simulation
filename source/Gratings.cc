// Gratings.cc -> Holds the functions that determine the diffraction after each of the gratings in the system
      ...
      for (int n=-lim; n<=lim; n++) 
        {
            ...
            {
                double dn =n-m; // Varname could be better. n, m, dn, dm are all vague. I STILL don't know what they represent. Something to ask Dr. McMorran about?I know they're used to compute a part of the grating interaction, and I think they're distances from something, but not sure what. 
                double dm = (m + n)/2;
...
// get intensity profile after two grating
{
  ...
  double eta2 = .4; //G2 open fraction; how open the second grating is.
    ...
    // Every. Single. Varname. God. varname varname varname. 
    double dn = 0;
    double dm =0;
    double m=0;
    double n=0;
    ...
    int RealorIm = 1; 
    ReTandImTgenerator(ReT,energy, elecOrAtom, RealorIm, vel, width, abszloc, accountGrav); // q is an array of 2 elements. Do both ReTgenerator and ImTgenerator affect q without it being overwritten?
    // NOPE. ReTgenerator just changes ReT, since arrays are essentially passed by reference in C.
    double ImT[41]={0};
    RealorIm = 2;
    ReTandImTgenerator(ImT,energy, elecOrAtom, RealorIm, vel, width, abszloc, accountGrav); // q is apparently reassigned here? Not used again after this line.
    // NOPE! q is useless, ImT is the one being changed here, since arrays are passed by reference.
